...
def problem2a(circle, rectangle, window):
    ...
    window.continue_on_mouse_click()
    upperrightpoint = rectangle.get_upper_right_corner()
    lowwerleftpoint = rectangle.get_lower_left_corner()
    line = rg.Line(upperrightpoint, lowwerleftpoint)
    line.arrow = 'last'
...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n):
        point1 = rg.Point(rect.get_upper_left_corner().x - k * delta,
                          rect.get_upper_left_corner().y - k * delta)
        point2 = rg.Point(rect.get_lower_right_corner().x + k * delta,
                          rect.get_lower_right_corner().y + k * delta)
